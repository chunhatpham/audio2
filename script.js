...
for(let i = 0; i < hotMovies.length; i++) {
    ...
    <div class="movie-card" onclick="openPlayer('${hotMovies[i].title}')">
        ...
        <div class="card-overlay">
            ...
            <span><i class="fas fa-link"></i> Lấy Link Ngay</span>
        </div>
    </div>`;
}
...
seriesData.forEach((series, index) => {
    ...
    for(let part = 1; part <= 3; part++) {
        ...
        <div class="movie-card" onclick="openPlayer('${series.title} - Phần ${part}')">
            ...
            <div class="card-overlay">
                <h3>${series.title}</h3>
                <span><i class="fas fa-link"></i> Lấy Link Tập Này</span>
            </div>
        </div>`;
    }
    seriesHTML += `
...
window.renderLatestMovies = function() {
    ...
    window.realMoviesDatabase.forEach(movie => {
        html += `<div class="movie-card" onclick="openPlayer('${movie.title}')">
            ...
            <div class="card-overlay"><h3>${movie.title}</h3><span style="color: #38ef7d;"><i class="fas fa-link"></i> Lấy Link ${movie.desc}</span></div>
        </div>`;
    });
    ...
    for (let i = startIndex; i <= endIndex; i++) {
        if (i < window.realMoviesDatabase.length) {
            ...
            html += `<div class="movie-card dynamic-page-card" onclick="openPlayer('${movie.title}')">
                ...
                <div class="card-overlay"><h3>${movie.title}</h3><span style="color: #38ef7d;"><i class="fas fa-link"></i> Lấy Link Tác Phẩm Này</span></div>
            </div>`;
        } else {
...
window.showAdProgressToast = function(currentStep) {
    ...
    if (currentStep < MAX_AD_CLICKS) {
        ...
        desc.innerText = "Nhiệm vụ hoàn tất! Hệ thống đang tạo link audio cho bạn...";
    ...
    window.adToastTimer = setTimeout(() => {
        closeAdToast();
    }, currentStep === MAX_AD_CLICKS ? 3000 : 4000);
};
...
window.openPlayer = function(movieName) {
    ...
    // Vẫn giữ tính năng quảng cáo 3 lần nếu không phải VIP
    if (!isVip && window.adUnlockStep < MAX_AD_CLICKS) {
        ...
        if (window.adUnlockStep < MAX_AD_CLICKS) {
            return; // Dừng lại, chưa cho mở khóa link
        }
    }
    
    window.adUnlockStep = 0; // Reset
    if(typeof triggerFloatingAds === 'function') triggerFloatingAds();

    // Phân tích Link Audio tương ứng với Phim
    let audioLink = "";
    let vipAudioLink = "https://files.catbox.moe/link-full-danh-cho-vip.m4a"; // Bạn có thể thêm mảng link VIP sau này
    
    if (movieName.includes('Chị Gái Loại Bỏ Tôi')) audioLink = "https://files.catbox.moe/9qspdw.m4a";
    ...
    else audioLink = "Hệ thống đang cập nhật link mới..."; 

    // Gán dữ liệu vào Modal Link
    document.getElementById('link-movie-img').src = imgSrc;
    document.getElementById('link-movie-title').innerText = movieName;
    document.getElementById('audio-link-input').value = audioLink;

    let vipArea = document.getElementById('vip-link-area');
    if(isVip) {
        vipArea.style.display = 'block';
        document.getElementById('vip-audio-link-input').value = vipAudioLink; 
    } else {
    ...
    // Cập nhật lịch sử
    setTimeout(() => { addHistoryEntry('watch', movieName, 'Đã lấy link', 0, imgSrc); }, 50);
};
...
window.copyAudioLink = function() {
    let copyText = document.getElementById("audio-link-input");
    copyText.select();
    copyText.setSelectionRange(0, 99999);
    navigator.clipboard.writeText(copyText.value).then(() => {
        showNotification('success', 'Thành Công', 'Đã sao chép link audio cơ bản thành công!', 'Tuyệt vời');
    });
};

window.copyVipAudioLink = function() {
    let copyText = document.getElementById("vip-audio-link-input");
    copyText.select();
    copyText.setSelectionRange(0, 99999);
    navigator.clipboard.writeText(copyText.value).then(() => {
        showNotification('success', 'Đặc Quyền VIP', 'Đã sao chép link audio FULL thành công!', 'Tuyệt vời');
    });
};

document.getElementById('link-modal-overlay').addEventListener('click', function(e) { 
    ...
    history.forEach((item, index) => {
        ...
        if (item.type === 'watch') {
            ...
            html += `<div class="history-item type-watch" style="animation-delay: ${delay}s;" onclick="openPlayer('${item.title}')">
                ...
                <div class="hist-right"><div class="hist-val-watch" style="color: var(--primary-color); border-color: var(--primary-color); font-weight: bold; background: rgba(229,9,20,0.1);"><i class="fas fa-link"></i> Lấy Lại Link</div></div>
            </div>`;
        } else {
...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', function() {
        navigator.serviceWorker.register('sw.js').catch(function(err) { console.log('ServiceWorker đăng ký thất bại: ', err); });
    });
}
