// ================= ĐÓNG THÔNG BÁO WELCOME =================
window.closeWelcomeModal = function() {
    document.getElementById('welcome-modal-overlay').classList.remove('show');
};
...
window.renderLatestMovies = function() {
    ...
    // Giới hạn trang chủ chỉ hiện đúng 10 phim mới nhất để mượt web
    ...
    // Cấp phát khung rỗng nếu không đủ 10
...
window.executeSearch = function() {
    ...
    // Tìm kiếm trong kho dữ liệu toàn cầu (bao gồm cả các phim bị ẩn trên Trang chủ)
    let foundIndex = window.realMoviesDatabase.findIndex(m => 
        normalizeVietnamese(m.title).includes(normalizedInput)
    );

    if (foundIndex !== -1) {
        let movie = window.realMoviesDatabase[foundIndex];
        
        // Mở tab danh sách để hiển thị phim nếu nó không có ở trang chủ
        openTab('tab-list');
        
        // Tính toán nó nằm ở trang số mấy và chuyển đến đó
        let targetPage = Math.floor(foundIndex / 10) + 1;
        goToPage(targetPage);
        
        // Đợi render xong rồi cuộn và bôi viền sáng
        setTimeout(() => {
        ...
        // Fallback: Tìm quét DOM ở Trang Chủ đối với các phim tĩnh (thịnh hành, top like...)
...
window.openPlayer = function(movieName) {
    ...
    // Logic nhiệm vụ click quảng cáo
    ...
    if (movieName.includes('Sự Kiện Cho Đàn Ông')) audioLink = "https://files.catbox.moe/o68ce7.m4a";
    ...
    // Thay đổi trạng thái nút Video dựa vào VIP
...
window.playCurrentAudio = function() {
    if(window.currentPlayingAudio && window.currentPlayingAudio.startsWith("http")) {
        window.open(window.currentPlayingAudio, '_blank');
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Hiển thị thông báo Welcome ngay khi vào
    setTimeout(() => {
        let welcomeModal = document.getElementById('welcome-modal-overlay');
