...
for(let i = 0; i < hotMovies.length; i++) {
    ...
    <div class="movie-card" onclick="openPlayer('${hotMovies[i].title}')">
        ...
        <div class="card-overlay">
            ...
            <span><i class="fas fa-play-circle"></i> Xem Phim Ngay</span>
        </div>
    </div>`;
}
...
seriesData.forEach((series, index) => {
    ...
    for(let part = 1; part <= 3; part++) {
        ...
        <div class="movie-card" onclick="openPlayer('${series.title} - Phần ${part}')">
            ...
            <div class="card-overlay">
                <h3>${series.title}</h3>
                <span><i class="fas fa-play-circle"></i> Xem Tập Này</span>
            </div>
        </div>`;
    }
    seriesHTML += `
...
// Biến lưu trữ link để chuyển hướng
...
window.openPlayer = function(movieName) {
    ...
    if (movieName.includes('Chấm Dứt Với Gia Đình')) audioLink = "https://files.catbox.moe/uz5ky7.m4a";
